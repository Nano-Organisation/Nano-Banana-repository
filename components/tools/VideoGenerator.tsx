...
import { Film, Download, RefreshCw, Lock, Smartphone, Monitor, Upload, Video, AlertTriangle, Clock } from 'lucide-react';
...
const VideoGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const url = await generateVideoWithGemini(finalPrompt, aspectRatio, inputImage || undefined, onRetry);
           ...
           {status === 'loading' ? (
              <div className="text-center space-y-4 p-8 bg-slate-900/50 border border-slate-800 border-dashed rounded-2xl">
                 ...
                 <div className="space-y-2">
                    ...
                    {retryMessage && (
                      <div className="flex items-center justify-center gap-2 text-amber-500 bg-amber-500/10 px-4 py-2 rounded-full border border-amber-500/20 animate-fade-in">
              ...
              <div className="flex flex-col items-center justify-center py-12 space-y-4 text-center p-6 bg-red-900/10 rounded-2xl border border-red-500/20">
                 ...
                 <div className="space-y-1"><h3 className="text-red-500 font-bold">Video Generation Failed</h3><p className="text-slate-400 text-sm">{errorMessage}</p></div>
              ...
              <div className="space-y-4 animate-fade-in-up">
                 <div className={`bg-black rounded-2xl overflow-hidden shadow-2xl border border-slate-800 relative group mx-auto ${aspectRatio === '9:16' ? 'aspect-[9/16] max-w-sm' : 'aspect-video w-full'}`}>
                    <video src={videoUrl} controls autoPlay loop muted playsInline className="w-full h-full object-contain" />
                 </div>
                 <a href={videoUrl} download={`nano-video-${Date.now()}.mp4`} className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors border border-slate-700"><Download className="w-5 h-5" />Download MP4</a>
              </div>
           ) : (
              <div className="text-center text-slate-500 border-2 border-dashed border-slate-800 rounded-2xl p-12 h-full flex flex-col items-center justify-center min-h-[400px]">
                 <Video className="w-16 h-16 opacity-20 mb-4" /><p>Your video creation will appear here.</p>
              </div>
           )}
        </div>
      </div>

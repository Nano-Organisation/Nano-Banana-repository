...
const StorybookLargeTool: React.FC = () => {
  ...
  const handleGenerate = async (isExtending = false) => {
    ...
    try {
      ...
      // --- LOGIC FIXES IMPLEMENTATION ---
      
      // 1. Location DNA Extraction (Persistence Map)
      // Stores specific environment descriptions for each location ID
      const locationManifest: Record<string, string> = {};
      
      // If extending, hydrate from existing pages first
      if (isExtending && existingPages.length > 0) {
         existingPages.forEach(p => {
            if (p.locationId && p.environmentDescription && !locationManifest[p.locationId]) {
               locationManifest[p.locationId] = p.environmentDescription;
            }
         });
      }
      ...
      for (let i = 0; i < script.pages.length; i++) {
          ...
          const locId = page.locationId || `loc_${i}`;
          
          // Fix 1: Enforce Location DNA
          // If this location ID is new, store its description. If known, override with stored description.
          if (!locationManifest[locId] && page.environmentDescription) {
              locationManifest[locId] = page.environmentDescription;
          }
          const envContext = locationManifest[locId] || page.environmentDescription || "A neutral background";

          // Fix 2: Character Invariants (Pip's Metamorphosis)
          // Look up active characters in the casting sheet using lenient partial matching
          // to catch cases where "Ghost" is in text but "Pip" is in casting sheet
          const pageTextLower = page.text.toLowerCase();
          const activeChars = (script.castingSheet || []).filter(c => {
             const charId = c.id.toLowerCase();
             // Match by explicit list OR text mention
             return (page.charactersPresent || []).some(p => p.toLowerCase() === charId) || 
                    pageTextLower.includes(charId);
          });
          
          // Fallback if no characters found but a main character description exists
          if (activeChars.length === 0 && script.characterDescription) {
              activeChars.push({ id: script.characterName || "Main", description: script.characterDescription });
          }

          const charPrompts = activeChars.map(c => 
              `SUBJECT ${c.id.toUpperCase()}: ${c.description}. (RULE: Keep appearance exactly consistent. Do not morph into human if described as creature/ghost.)`
          ).join(' ');

          // Fix 4: Negative Prompts (Phantom Candle)
          const negative = "clutter, mess, extra furniture, extra candles, distorted hands, text, speech bubbles, blurry, watermark, bad anatomy, mutation, morphing, human child (if ghost), floating objects, inconsistent architecture";

          // Fix 3: Action Swapping Logic (Structured Prompting)
          // Explicitly separating Setting, Characters, and Action to prevent attribute bleeding.
          const fullPrompt = `
            STYLE: ${script.style}.
            LOCATION DNA (BACKGROUND): ${envContext}.
            CHARACTERS (FOREGROUND): ${charPrompts}.
            ACTION SCENE: ${page.imagePrompt}.
            COMPOSITION: ${page.stageDirections || "Wide shot, clear focus"}.
            NEGATIVE_PROMPT: ${negative}.
          `.replace(/\s+/g, ' ').trim();

          const imageUrl = await drawPage(i, fullPrompt, characterReferenceImage);
             ...
             <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                    <button onClick={() => { setSelectedCharacterIds([]); setShowCharManager(true); }} className={`flex-shrink-0 p-3 rounded-xl border flex flex-col items-center justify-center w-24 h-24 transition-all bg-slate-800 border-slate-700 text-slate-500 hover:bg-slate-700 hover:text-white`}>
